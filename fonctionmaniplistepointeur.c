...
{
    return (int)(liste==(PTliste*)NULL); //comparer la tete de la liste avec Null
}
...
{
    PTliste *courant =NULL; //declarer un pointeur qui va parcourir toutes la liste
...
PTliste *inserer_debut(PTliste *liste , int donnee)
{
    ...
    NE->suivant=liste ;
    liste=NE;
    return (PTliste*)(liste); //returner la liste
...
PTliste *inserer_fin(PTliste* liste , int donnee)
{
    ...
    PTliste *NE=creerliste(),*courant ;
    NE->entier=donnee;//remplir le champs donnee
    //verifier si on a une liste vide , si oui on effectuer une insertion a la fin
    if(estvide(liste)) return(PTliste*)(liste=inserer_debut(liste,donnee));
    for(courant=liste;courant->suivant;courant=courant->suivant);//chercher la fin de la liste
